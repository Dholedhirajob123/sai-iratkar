...
const getGameStatus = (openTime: string, closeTime: string): { openStatus: GameStatus; closeStatus: GameStatus } => {
  const now = new Date();
  ...
  const readyDate = new Date(openDate.getTime() - 15 * 60000);

  let openStatus: GameStatus = "timeout";
  let closeStatus: GameStatus = "timeout";

  if (now >= readyDate && now < openDate) openStatus = "ready";
  else if (now >= openDate && now < closeDate) openStatus = "open";

  if (now >= openDate && now < closeDate) closeStatus = "open";
...
const statusConfig = {
  ready: { label: "Ready", icon: Zap, className: "text-success border-success/30 bg-success/10" },
  open: { label: "Open", icon: Play, className: "text-success border-success/30 bg-success/10" },
  timeout: { label: "Time Out", icon: Clock, className: "text-muted-foreground border-foreground/10 bg-accent opacity-50" },
};
...
const GameCard = ({ game, onPlayOpen, onPlayClose }: GameCardProps) => {
  ...
  if (!isActive) {
    return (
      <div className="surface-card p-0 overflow-hidden opacity-60">
        <div className="bg-accent/50 border-b-2 border-foreground/10 px-4 py-3">
          <h3 className="text-center font-mono font-bold text-sm text-foreground tracking-widest">{game.name}</h3>
        </div>
  ...
  return (
    <div className="surface-card p-0 overflow-hidden">
      ...
      <div className="flex border-t-2 border-foreground/10">
        {[{ status: openStatus, label: "Open", handler: () => onPlayOpen(game) }, { status: closeStatus, label: "Close", handler: () => onPlayClose(game) }].map(({ status, label, handler }, i) => {
          ...
          return (
            <button
              ...
              onClick={handler}
              disabled={disabled}
              className={`flex-1 flex items-center justify-center gap-2 py-3 font-mono text-xs font-semibold transition-opacity ${cfg.className} ${i === 0 ? "border-r-2 border-foreground/10" : ""} ${disabled ? "cursor-not-allowed" : "hover:opacity-80"}`}
            >
